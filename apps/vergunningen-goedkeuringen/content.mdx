---
title: Vergunningen goedkeuringen
date: 2026-06-01
summary: Analyse van de goedgekeurde bouwvergunningen.
tags: [vergunningen, bouw]
slug: vergunningen-goedkeuringen
sourceProvider: Statbel
sourceTitle: Bouwvergunningen per gemeente
sourceUrl: https://statbel.fgov.be/nl/themas/bouwen-wonen/bouwvergunningen
sourcePublicationDate: 2026-04-30
---
Hieronder vindt u de analyse van de goedgekeurde vergunningen voor residentiële gebouwen (data: februari 2026; data beschikbaar tot en met 28/02/2026), opgesplitst in renovatie en nieuwbouw.
